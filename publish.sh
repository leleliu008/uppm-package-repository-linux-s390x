...
if [ -d uppm-formula-repository-linux-s390x/.git ] ; then
    cd  uppm-formula-repository-linux-s390x
    gh repo sync
    cd -
else
    gh repo clone leleliu008/uppm-formula-repository-linux-s390x
fi
...
do
    ...
    PACKAGE_NAME=$(printf '%s\n' "${filename%-linux-s390x.tar.xz}" | sed 's|\(.*\)-\(.*\)|\1|')

    PACKAGE_VERSION=$(printf '%s\n' "${filename%-linux-s390x.tar.xz}" | sed 's|\(.*\)-\(.*\)|\2|')

    PACKAGE_BIN_SHA=$(sha256sum "package/$filename" | cut -d ' ' -f1)

    PACKAGE_BIN_URL="https://github.com/leleliu008/uppm-package-repository-linux-s390x/releases/download/${RELEASE_VERSION}/${filename}"

    UPPM_PACKAGE_FORMULA_FILEPATH="uppm-formula-repository-linux-s390x/formula/$PACKAGE_NAME.yml"
...
run tar vxf package/gtar-*-linux-s390x.tar.xz -C "$TEMP_DIR" --strip-components=1
run tar vxf package/gzip-*-linux-s390x.tar.xz -C "$TEMP_DIR" --strip-components=1
run tar vxf package/xz-*-linux-s390x.tar.xz   -C "$TEMP_DIR" --strip-components=1
